...
def single_image_save(samples, path):
  images = inverse_transform(samples)
  bwboolean = images.shape[-1] == 1
  for i in range(images.shape[0]):
    if bwboolean:
      scipy.misc.imsave(path + "image" + str(i) + ".png", images[i, :, :, 0])
    else:
      scipy.misc.imsave(path + "image" + str(i) + ".png", images[i, :, :, :])
  np.save(file = path + "images.npy", arr = images)
